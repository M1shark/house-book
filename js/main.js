...
document.addEventListener("DOMContentLoaded", () => {
  const tabs = document.querySelector(".reproductions__container");
  const tabsBtn = document.querySelectorAll(".reproductions__tab");
  const tabsContent = document.querySelectorAll(".reproductions__content");
  tabs.addEventListener("click", e => {
    if (e.target.classList.contains("reproductions__tab")) {
      const tabsPath = e.target.dataset.tabsPath;
      console.log(tabsPath);
      tabsBtn.forEach(el => {
        el.classList.remove("reproductions__tab--active");
      });
      document.querySelector(`[data-tabs-path="${tabsPath}"]`).classList.add("reproductions__tab--active");
      tabsHandler(tabsPath);
    }
  });
  const tabsHandler = path => {
    tabsContent.forEach(el => {
      el.classList.remove("reproductions__content--active");
    });
    document.querySelector(`[data-tabs-target="${path}"]`).classList.add("reproductions__content--active");
  };
});
